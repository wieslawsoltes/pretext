...
internal static class JustificationComparisonData
{
    ...
    public static readonly Dictionary<string, string[]> HyphenExceptions = new(StringComparer.OrdinalIgnoreCase)
    {
    ...
    [
        "anti", "auto", "be", "bi", "co", "com", "con", "contra", "counter", "de", "dis", "en", "em", "ex", "extra", "fore", "hyper", "il", "im", "in",
        "inter", "intra", "ir", "macro", "mal", "micro", "mid", "mis", "mono", "multi", "non", "omni", "out", "over", "para", "poly", "post", "pre",
        "pro", "pseudo", "quasi", "re", "retro", "semi", "sub", "super", "sur", "syn", "tele", "trans", "tri", "ultra", "un", "under",
    ];

    public static readonly string[] Suffixes =
    [
        "able", "ible", "tion", "sion", "ment", "ness", "ous", "ious", "eous", "ful", "less", "ive", "ative", "itive", "al", "ial", "ical", "ical", "ing",
        "ling", "ed", "er", "est", "ism", "ist", "ity", "ety", "ty", "ence", "ance", "ly", "fy", "ify", "ize", "ise", "ure", "ture",
    ];
    ...
    public static string[] HyphenateWord(string word)
    {
        ...
        if (HyphenExceptions.TryGetValue(lower, out var exceptionParts))
        {
            return ReconstructWordParts(word, exceptionParts);
        }

        foreach (var prefix in Prefixes)
        {
            if (lower.StartsWith(prefix, StringComparison.Ordinal) && lower.Length - prefix.Length >= 3)
            {
                return [word[..prefix.Length], word[prefix.Length..]];
            }
        }

        foreach (var suffix in Suffixes)
        {
            if (lower.EndsWith(suffix, StringComparison.Ordinal) && lower.Length - suffix.Length >= 3)
            {
                var cut = word.Length - suffix.Length;
    ...
    private static string[] ReconstructWordParts(string word, IReadOnlyList<string> parts)
    {
        var rebuilt = new List<string>(parts.Count);
        var position = 0;
        foreach (var part in parts)
        {
            rebuilt.Add(word.Substring(position, Math.Min(part.Length, word.Length - position)));
            position += part.Length;
        }

        if (position < word.Length && rebuilt.Count > 0)
        {
            rebuilt[^1] += word[position..];
        }

        return rebuilt.Where(static part => part.Length > 0).ToArray();
    }
}
